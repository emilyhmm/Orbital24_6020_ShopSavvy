    ...
    try {
        ...
        while (!isNextDisabled) {
            ...
            for (const i of productHandles) {
                ...
                try {
                    title = await page.evaluate(el => el.querySelector("h2 > a > span").textContent.trim(), i);
                } catch (error) {}

                try {
                    price = await page.evaluate(el => el.querySelector(".a-price > .a-offscreen").textContent.trim(), i);
                } catch (error) {}
